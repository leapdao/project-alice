...
export default class WalletTabPanel extends React.Component<
  WalletTabPanelProps
> {
  consolidateAddress = async () => {
    ...
    if (unspent.length > 1 && store && (store as any).tcs[store.color]) {
      ...
      const balance = Number(
        await contract.methods.balanceOf(store.address).call()
      );
  ...
  handleSendTransaction = async (to: string, value: number) => {
    ...
    const hash = await new Promise((resolve, reject) => {
      web3.eth
        .sendSignedTransaction(tx.toRaw() as any, (err, txHash) => {
          if (err) {
            reject(err);
          } else {
            store.add({
    ...
    return (
      <div className="alice-tab-panel">
        ...
        <TransactionsPanel
          ...
          addresses={Object.keys(stores).reduce(
            (o, key) => ({
              ...o,
...
export const createWalletTabPanel = (wallet: string) =>
  inject((store: any) => ({
    store: store[wallet],
    stores: store,

...
export default class App extends React.PureComponent<any, State> {
  ...
  onChangeToken = (token: Token) => {
    const color = this.state.tokens.findIndex(
      t => t.token.options.address === token.token.options.address
    );
  ...
  render() {
    ...
    const color = token
      ? tokens.findIndex(t => t.token.options.address === token.options.address)
      : 0;
